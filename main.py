...
def dibujar_grafo(ruta):
    ...
    for ciudad in coordenadas:
        G.add_node(ciudad, pos=(coordenadas[ciudad][1], coordenadas[ciudad][0]))  # (lon, lat)

    # Agregar aristas
    for ciudad, vecinos in grafo.items():
        for vecino, distancia in vecinos.items():
            G.add_edge(ciudad, vecino, weight=round(distancia, 1))
    ...
    plt.title('Grafo de Ciudades y Ruta Óptima')
    plt.axis('off')
    plt.tight_layout()
    ...
    plt.savefig('static/grafo.png')  # Guardar imagen
    plt.close()
...
def caracteristicas():
    ...
    # Dibujar el grafo resaltando la ruta
    dibujar_grafo(ruta)
